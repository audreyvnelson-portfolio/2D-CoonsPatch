...
class pts {
 ...
 pts drawAffineCopy(pt A0, pt B0, pt C0, pt A1, pt B1, pt C1) {
   beginShape(); 
   for (int v=0; v<nv; v++) map(G[v],A0,B0,C0,A1,B1,C1).v(); // see 01b in morph
   endShape(); 
   return this;
   }

//01a
 pts drawSpiralCopy(float t, float a, float s, pt F) {
   beginShape(); 
   for (int v=0; v<nv; v++) Spiral(G[v],t,a,s,F).v(); // see 01b in morph
   endShape(); 
   return this;
   }
